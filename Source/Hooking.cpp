...
#define PlayerCount PLAYER::GET_NUMBER_OF_PLAYERS()
#define GTAVersion NETWORK::_GET_ONLINE_VERSION()
...
namespace Mod_Hub_Base
{
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with P-Ice";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "Loading with P-Ice";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with P-Ice";
		if (std::strcmp(label, "NT_INV_MP_SAVE") == 0) return "Joining ~g~friend~w~?";
		//if (std::strcmp(label, "PM_SCR_MAP") == 0) return "~r~MAP";
		//if (std::strcmp(label, "PM_SCR_FRI") == 0) return "~b~FRIENDS";
		//if (std::strcmp(label, "PM_SCR_GAL") == 0) return "~p~GALLERY";
		//if (std::strcmp(label, "PM_SCR_GAL") == 0) return "~g~INFO";
		//if (std::strcmp(label, "PM_SCR_MIS") == 0) return "~~y~ONLINE";
		//if (std::strcmp(label, "PM_SCR_SET") == 0) return "~o~SETTINGS";
		//if (std::strcmp(label, "PM_SCR_BRF") == 0) return "~b~BRIEF";
		//if (std::strcmp(label, "PM_SCR_MAP") == 0) return "g~STATS";
		//if (std::strcmp(label, "PM_SCR_STA") == 0) return "Scooby Radio";
		if (std::strcmp(label, "RESPAWN_W_MP") == 0) return "Imagine dying...";
		//if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Scooby Menu";
		//if (std::strcmp(label, "PM_INF_PGOB0") == 0) return "Scooby Menu";
		//if (std::strcmp(label, "PM_PAUSE_HDR") == 0) return "Scooby Mod Menu";
		//if (std::strcmp(label, "PM_INF_STOREB") == 0) return "~r~Buy Shark Cards with real money.";
		//if (std::strcmp(label, "PM_GO") == 0) return "~y~Go Online With Scooby ∑";
	...
	*/
	...
	void Hooking::FindPatterns()
	{
		...
		//setPattern <Bytes24>("SCOOBY INJECTED (F5 TO OPEN)",
		...
		//Log::Msg("[ Getting Start Up Game Label Text... ]");
...
//Log::Msg("is this cool ^^^ no as every paste has this on the looder or menu log like this Ni****");
//
//
//Log::Msg("Btw Yes scoobypaste is injected");
